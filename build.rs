...
fn main() {
    if var("CARGO_FEATURE_LOGIND").ok().is_none() && var("CARGO_FEATURE_LIBSEAT").ok().is_none() {
        println!("cargo:warning=You are compiling lime_wm without logind/libseat support.");
        ...
        println!("cargo:warning=$ cd lime_wm; cargo run --feature logind");
        println!("cargo:warning=To enable libseat support add `--feature libseat` to your cargo invocation.");
        println!("cargo:warning=$ cd lime_wm; cargo run --feature libseat");
    }
}
