...
#[cfg(feature = "egl")]
delegate_dmabuf!(LimeWmState<UdevData>);
...
pub fn run_udev(log: &Logger) {
     ...
     */
    let primary_gpu = if let Ok(var) = std::env::var("LIME_WM_DRM_DEVICE") {
    ...
    let mut state = LimeWmState::init(&mut display, event_loop.handle(), data, log.clone(), true);
...
impl Drop for SurfaceData {
    fn drop(&mut self) {
        if let Some(global) = self.global.take() {
            self.dh.remove_global::<LimeWmState<UdevBackend>>(global);
        }
    }
}
...
fn scan_connectors(
    ...
    gbm: &Rc<RefCell<GbmDevice<SessionFd>>>,
    display: &mut Display<LimeWmState<UdevData>>,
    space: &mut Space,
    signaler: &Signaler<SessionSignal>,
    logger: &::slog::Logger,
...
fn render_surface(
    ...
    let render_res =
        crate::render::render_output(&output, space, renderer, age.into(), &*elements, logger)
            .map(|x| x.is_some());
