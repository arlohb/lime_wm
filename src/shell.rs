...
use smithay::{
    ...
    desktop::{
        ...
        PopupManager, PopupPointerGrab, PopupUngrabStrategy, Space, Window, WindowSurfaceType,
    },
    reexports::{
    ...
    wayland::{
        ...
        compositor::{
            with_states, with_surface_tree_upward, CompositorHandler, CompositorState,
            TraversalAction,
        },
        output::Output,
        seat::{
        ...
        shell::{
            wlr_layer::{
                Layer, LayerSurface as WlrLayerSurface, LayerSurfaceAttributes,
...
use crate::state::{Backend, LimeWmState};
...
impl<BackendData> PointerGrab<LimeWmState<BackendData>> for MoveSurfaceGrab {
    fn motion(
        &mut self,
        data: &mut LimeWmState<BackendData>,
        _dh: &DisplayHandle,
    ...
    fn button(
        &mut self,
        _data: &mut LimeWmState<BackendData>,
        _dh: &DisplayHandle,
        handle: &mut PointerInnerHandle<'_, LimeWmState<BackendData>>,
        event: &ButtonEvent,
    ) {
...
impl<BackendData> PointerGrab<LimeWmState<BackendData>> for ResizeSurfaceGrab {
    fn motion(
        &mut self,
        _data: &mut LimeWmState<BackendData>,
        _dh: &DisplayHandle,
...
impl<BackendData> BufferHandler for LimeWmState<BackendData> {
    fn buffer_destroyed(&mut self, _buffer: &WlBuffer) {}
}

impl<BackendData: Backend> CompositorHandler for LimeWmState<BackendData> {
...
impl<BackendData: Backend> XdgShellHandler for LimeWmState<BackendData> {
    ...
    fn move_request(
            ...
            .contains(xdg_toplevel::State::Maximized)
        {
            ...
            //
            // but for lime_wm needs setting location to pointer location is fine
...
impl<BackendData> WlrLayerShellHandler for LimeWmState<BackendData> {
